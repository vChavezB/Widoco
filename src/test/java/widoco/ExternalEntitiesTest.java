...
public class ExternalEntitiesTest {
    ...
    public void testExternalEntityOntology() {
        ...
        try{
            ...
            // Debug output for test-side diagnostics
            System.out.println("DEBUG: CrossRef file absolute path: " + crossRefFile.getAbsolutePath());
            System.out.println("DEBUG: CrossRef file exists: " + crossRefFile.exists());
            System.out.println("DEBUG: CrossRef file length: " + crossRefFile.length() + " bytes");
            if (crossRefFile.exists() && crossRefFile.length() > 0) {
                try {
                    String content = new String(java.nio.file.Files.readAllBytes(crossRefFile.toPath()));
                    int previewLength = Math.min(2000, content.length());
                    System.out.println("DEBUG: CrossRef content preview (first " + previewLength + " chars):");
                    System.out.println(content.substring(0, previewLength));
                } catch (Exception e) {
                    System.out.println("DEBUG: Error reading crossref file content: " + e.getMessage());
                }
            }
